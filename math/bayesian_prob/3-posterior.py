...
def posterior(x, n, P, Pr):
    ...
    Args:
        ...
        Pr (np.ndarray): 1D array of prior probabilities (same shape)

    Returns:
        np.ndarray: posterior probabilities for each p in P

    Raises:
        ValueError or TypeError with messages as specified.
    """
    ...
    # 6. All values in P and Pr must be in [0, 1] (check P first)
    ...
    def log_factorial(k):
        if k <= 1:
            return 0.0
        vals = np.arange(1, k + 1, dtype=float)
    ...
    # Compute log-likelihood terms safely (handle edge cases)
    if x == 0:
    ...
    # posterior: intersections / marginal (NumPy array)
